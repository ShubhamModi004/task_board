...
const TaskStatus = () => {
  ...
  const selectedValue = useMemo(() => {
    return statuses.find(item => item.title.toUpperCase() === status) || statuses[0];
  }, [status]);

  return (
    <DropdownPill onClick={handleSelection} image={selectedValue?.image} items={statuses} placeholder={selectedValue?.title} />
  )
}
